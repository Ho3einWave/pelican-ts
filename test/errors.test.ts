...
describe('PelicanError', () => {
  it('should set status, code, and message from first error', () => {
    const err = new PelicanError(404, [
  ...
  it('should handle empty errors array', () => {
    const err = new PelicanError(500, []);
  ...
  it('should be an instance of Error', () => {
    const err = new PelicanError(400, [{ code: 'Bad', status: '400', detail: 'bad request' }]);
    expect(err).toBeInstanceOf(Error);
    expect(err).toBeInstanceOf(PelicanError);
  });
});

describe('PelicanValidationError', () => {
  it('should extract field errors from source', () => {
    const err = new PelicanValidationError([
      {
        code: 'required',
        status: '422',
    ...
    expect(err.name).toBe('PelicanValidationError');
  ...
  it('should be an instance of PelicanError', () => {
    const err = new PelicanValidationError([]);
    expect(err).toBeInstanceOf(PelicanError);
  });
});

describe('PelicanRateLimitError', () => {
  it('should store retryAfter value', () => {
    const err = new PelicanRateLimitError(
      [{ code: 'TooManyRequests', status: '429', detail: 'Rate limited' }],
      30,
    );
    expect(err.status).toBe(429);
    expect(err.retryAfter).toBe(30);
    expect(err.name).toBe('PelicanRateLimitError');
  });

  it('should be an instance of PelicanError', () => {
    const err = new PelicanRateLimitError([], 60);
    expect(err).toBeInstanceOf(PelicanError);
  });
});
